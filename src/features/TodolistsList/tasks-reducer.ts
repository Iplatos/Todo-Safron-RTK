...
export const fetchTasksTC = (todolistId: string): AppThunk => (dispatch: Dispatch) => {
  ...
  todolistsAPI.getTasks(todolistId)
    .then((res) => {
      ...
      dispatch(tasksReducerActions.setTasksAC({ tasks, todolistId }));
...
export const removeTaskTC = (taskId: string, todolistId: string): AppThunk => (dispatch: Dispatch) => {
  todolistsAPI.deleteTask(todolistId, taskId)
    .then(res => {
      const action = tasksReducerActions.removeTaskAC({ taskId, todolistId });
      dispatch(action);
    });
};
  ...
  todolistsAPI.createTask(todolistId, title)
    .then(res => {
      if (res.data.resultCode === 0) {
        ...
        const action = tasksReducerActions.addTaskAC({ task });
        dispatch(action);
        dispatch(appActions.setAppStatus({ status: "succeeded" }));
      } else {
...
export const updateTaskTC = (taskId: string, domainModel: UpdateDomainTaskModelType, todolistId: string): AppThunk =>
  (dispatch, getState: () => AppRootStateType) => {
    ...
    todolistsAPI.updateTask(todolistId, taskId, apiModel)
      .then(res => {
        if (res.data.resultCode === 0) {
          const action = tasksReducerActions.updateTaskAC({ taskId, model: domainModel, todolistId });
          dispatch(action);
        } else {
          handleServerAppError(res.data, dispatch);
        }
